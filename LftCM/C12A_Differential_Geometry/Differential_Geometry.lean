...
@[simp] lemma neg_mem_Ioo_minus_one_one (x : ℝ) :
  ...
  sorry
-- SOLUTIONS:
@[simp] lemma neg_mem_Ioo_minus_one_oneαα (x : ℝ) :
    -x ∈ Ioo (-1 : ℝ) 1 ↔ x ∈ Ioo (-1 : ℝ) 1 := by
...
-- SOLUTIONS:
def myFirstLocalHomeoαα : LocalHomeomorph ℝ ℝ where
  toFun := fun x ↦ -x
  invFun := fun x ↦ -x
  source := Ioo (-1) 1
  target := Ioo (-1) 1
...
def mySecondLocalHomeo : LocalHomeomorph ℝ ℝ where
  ...
  continuous_invFun := sorry
-- SOLUTIONS:
def mySecondLocalHomeoαα : LocalHomeomorph ℝ ℝ where
  toFun := fun x ↦ if x = 3 then 0 else - x
  invFun := fun x ↦ -x
  source := Ioo (-1) 1
  target := Ioo (-1) 1
...
-- EXAMPLES:
theorem diffeomorph_of_one_dim_compact_connectedαα :
    sorry :=
  sorry
-- SOLUTIONS:
